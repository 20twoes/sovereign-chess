...
class Board extends StatelessWidget {
  final List<String> ranks = List<String>.generate(
    12,
    (int index) => '${index + 1}',
    growable: false,
  );

  final List<String> files = List<String>.generate(
    12,
  ...
  List<Widget> _generateSquares() {
    ...
    for (final (rank_index, rank) in ranks.reversed.indexed) {
      for (var (file_index, file) in files.indexed) {
        list.add(Square('$file$rank', getColor(rank_index, file_index)));
      }
    }
    return list;
  }
...
class Square extends StatelessWidget {
    ...
    const Square(this.text, this.color, {super.key});

    @override
    Widget build(BuildContext context) {
      final bgColor = color == 'dark' ? Colors.teal[200] : Colors.teal[100];
      return Container(
        color: bgColor,
        child: Text(text),
      );
